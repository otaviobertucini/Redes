...
/* ******************************************************************
 ALTERNATING BIT AND GO-BACK-N NETWORK EMULATOR: VERSION 1.1  J.F.Kurose
   ...
   data transfer protocols (from A to  Bidirectional transfer of data
...
// Enfileira o pacote novo que foi recebido
void handle_received_message(struct msg message)
{
  struct pkt *packet = &buffer[next_packet];
  packet->seqnum = next_packet;
...
void A_output(struct msg message)
{
  ...
  // Verifica se o buffer nao esta cheio
  if (next_packet - waitingA >= BUFFSIZE)
  {
    return;
  }
...
void B_input(struct pkt packet)
{
  ...
  {
    struct pkt to_send;
    to_send.acknum = waitingB - 1;
    to_send.seqnum = 0;
    to_send.checksum = calc_checksum(to_send);
    tolayer3(B, to_send);
    return;
  }
